...
import { MovementRestrictions, RestrictedMove } from "../pieces/MovementHelper";
...
export class GameState {
  ...
  constructor(
    ...
    this.movementRestrictions = this.getAllMovementRestrictions();
  }
  ...
  getAllMovementRestrictions(): MovementRestrictions[] {
    ...
    const pieces = this.board.pieces.flat().filter(p => !!p);
  ...
  GetRestrictedMoves(moves: Move[]): RestrictedMove[] {
    const results: RestrictedMove[] = [];
    for (const move of moves) {
      const match = this.movementRestrictions.find((r) =>
