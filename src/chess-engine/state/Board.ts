...
export class Board {
    ...
    constructor(width: number, height: number, defaultTileFactory?: () => Tile) {
        this.width = width;
        this.height = height;

        this.pieces = Array.from({ length: width }, () => Array(height).fill(null));
        
        // Create default tiles - we'll need a factory since we don't have StandardTile yet
        // For now, we'll require it to be passed in or use a placeholder
        const createDefaultTile = defaultTileFactory || (() => {
            // Placeholder - will be replaced when we port tiles
            return { id: "", position: new Vector2Int(0, 0), clone: () => createDefaultTile() } as Tile;
        });
        
        this.tiles = Array.from({ length: width }, () =>
            Array.from({ length: height }, () => createDefaultTile())
        );
    ...
    getTile(pos: Vector2Int): Tile {
        return this.isInBounds(pos) ? this.tiles[pos.x][pos.y] : this.tiles[0][0]; // Fallback
    }
